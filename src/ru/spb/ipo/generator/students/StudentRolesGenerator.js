...
export class StudentRolesGenerator extends BaseGeneratorUI {
    ...
    attachEvents(container) {
        ...
        container.querySelector('#generate-btn').onclick = () => {
            ...
            if (isNaN(N) || N < 1) {
                alert("Ошибка: Введите корректное число студентов.");
                return;
            }
            if (K === 0) {
                alert("Ошибка: Добавьте хотя бы одну должность.");
                return;
            }
            if (K > N) {
                alert(`Ошибка: Должностей (${K}) больше, чем студентов (${N}).`);
                return;
            }
